...
class UserProfileController extends Controller {
    ...
    public function updateNameUser(Request $req) {
    	...
    	$address = $req->address;
	    try {
	    	DB::update('update users set full_name = ? where id = ?',[$name, Auth::user()->id]);
	    	DB::update('update users set phone = ? where id = ?',[$phone_number, Auth::user()->id]);
    ...
    public function updateUserImage(Request $req) {
    	...
    	if($image = $req->file('image')){
	    	...
	    	$image -> move("img/user_profile/", $new_name);
	    }
	    try {
    		$old_image_name = DB::table('users')->select('photo')->where('id', Auth::user()->id)->get();
      		DB::update('update users set photo = ? where id = ?',[$new_name, Auth::user()->id]);
